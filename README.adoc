...
Under `~/.config/anime-rss` create a text file called `title`. It should contain a bunch of lines that match the anime title to download from https://subsplease.org. Each line is interpreted as a regex that describes one anime to download.
...
[IMPORTANT]
====
The lines need to (sub-)match the title in the RSS feed. You can find the titles with the following command (using https://github.com/kislyuk/yq?tab=readme-ov-file#xml-support[`xq`] provided by https://github.com/kislyuk/yq[yq]) to help you figure out to match:
